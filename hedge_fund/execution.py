...
def simulate_hybrid_trades(watchlist, intraday_data, daily_data,
    ...
    for today in all_trading_days:
        ...
        for ticker, pos in positions.items():
            ...
            if pos['direction'] == 'LONG':
                # SL check — compute ACTUAL R-multiple from current SL position
    ...
    if all_trading_days:
        ...
        for ticker, pos in positions.items():
            if ticker in daily_data:
                ...
                if p:
                    r = _calc_r(pos, p)
                    sized_pnl = (r - pos['cost_r'] + pos.get('partial_pnl', 0)) * pos.get('size', 1.0)
                    trades.append((sized_pnl, False, pos.get('size', 1.0), ticker, pos['direction']))
